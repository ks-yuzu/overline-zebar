...
stampが窓の起点より後ろの組だけを集約する。**前の窓のresetはこの窓の起点そのもの**
なので、追いついた台だけがちょうど残る。
...
**10ptより低い所からの下降はresetとみなさない。**gaugeは整数なので、低い読みでは
ほとんどの下降が「半分より下」を満たす (1→0も3→1も)。resetと読むと落ちた先が
もう一度乗る。実測のwindow内resetは45・24・73から落ちている。

**resetは、そこに配るものが無くても配分の境界にする。**0まで落ちたresetは上昇を
記録しないため、境界にしないと次の上昇が**resetをまたいだ区間のコスト**で割られ、
reset前に終わっていたsessionへreset後のクォータが付く。

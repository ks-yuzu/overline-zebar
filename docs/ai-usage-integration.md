...
1ptだけ下がることがあり、それをresetと読むと残りの値がもう一度乗る (84→81を
resetと読むと165%になる)。**resetは0付近まで落ち、修正は水準の近くに留まる。**
実測でreset後は0・0・5、修正は前の値の0.92〜0.99倍だった。
**下げ幅で測ると2種類が数ptしか離れず** (1・1・1 対 19・45・73)、閾値の1pt上の
修正がresetとして通る。
**この規則が偽になる観測:** 前の値の半分より下へ落ちる下方修正、または半分以上で
止まるreset。
...
**resetは、0まで落ちたかどうかに関わらず配分の境界にする。**resetの後に在る値は
resetから後に積んだ分なので、reset前のコストで割ると、reset前に終わっていた
sessionへreset後のクォータが付く。0まで落ちたresetは上昇を記録せず、0でない
resetは記録する (実測の`24→5`) ので、**片方だけを見る書き方では後者が漏れる。**

**reset自身の5分の刻みに載ったコストは、読める側へ残す。**resetはその刻みのどこかで
起きており、同じ刻みのコストがreset前か後かは分からない。両側から外すと行から消え、
**記録されている消費について画面が「No spend recorded」と言う。**境界を1刻み手前に
引いて、曖昧な1刻みはreset後の上昇へ回す。
...
実測で修正は1ptで週に1回ほど出る (34→33の翌日、7Dの窓は`total 37`に対しgauge 36)。
返すには、既に付けたsessionからその1ptを引く必要がある。**週に1ptのために
機構を1つ増やさない。**再測する:
...
経路を素通りしてtracebackで終わる。**レビュー3巡で3件 (label setを黙って落とす /
...
parser自身の誤りや`MemoryError`まで飲み込む。しかも**quota側は失敗を握り潰し、
cost側はcacheごと落とすので、同じ誤りが「古いcache」と「クォータの無い新しいcache」
のどちらにもなる。**どちらも更新が成功したように見える。
...
(dict、None、長さ1、3要素) はすべて`CostError`へ倒れてクォータが出ない、という
画面から見える失敗になる。**strだけが「読めなかった」ではなく「別の答え」へ倒れ、
捏造した数字で新しいcacheを公開する。**組はlistであって長さ2であることを要求する。

**長さも見る。**見ないと`[1, 2, 3]`はmatrixでは`CostError`、vectorでは添字1が
読めるので通る。**同じ応答が、当たったqueryの種類で「古いcacheへ落ちる」と
「新しいcacheを公開する」に分かれる。**catch-allを外した理由と同じ形である。

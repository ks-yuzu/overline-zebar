...
画面が静かになったら止め、上限で打ち切る。上限は、収まらない画面が cron の
`timeout 60s` に達して読み取りごと失われるのを防ぐためにある。実測で13秒だった。

**節の範囲は見出し直下の数行に限る。** 画面には、こちらのwindowに属さない数字がある。
下部のcredits枠は自前の`100% 100% used`と`Resets Oct 1`を持ち、部分再描画は見出しの
無い数字を残す。数字が来る前に描かれた見出しがそこまで届くと、**形の揃った、
もっともらしい、誤ったwindowが公開される。**

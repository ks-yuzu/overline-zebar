...
- **今のpaceが続いた場合の予測を、card下端の1行とwindow内graphの破線で示す。**
  ...
  - **未開始のwindowには出さない。**判定は軸と同じ`windowStarted`による。未開始の
    windowが報告するresetは滑動しており、graphは軸をそこへ固定せず直近の時間へ
    退避する。したがって点を置くと軸の外に出て、**cardだけがgraphに描けない時刻を
    述べる**状態になる。枠はresetを跨ぐため、使用率0%のwindowでも前のwindowの消費で
    「80時間後に尽きる」と述べうる。これが塞ぐ経路である。
  - **reset直後のwindowには出す。**`justReset`が真なら軸はresetに固定されており、
    **前のwindowを空にしたpaceをそのまま引き継いで描ける。**quotaが配り直された
    直後は、その読みが最も有用である。使用率0%だけで黙らせるとここまで巻き込む。
